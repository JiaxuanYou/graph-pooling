...
class graph_dataset_link_prediction(torch.utils.data.Dataset):
    def __init__(self, name = 'cora', test_ratio = 0.2):
        ...
        self.mask_train = (self.mask_train > (1-num_positive_train/(self.n*self.n))).astype(int)
        num_positive_test = np.sum((self.adj_test>0).astype(int))
        self.mask_test = self.adj_test + np.random.rand(self.n, self.n)
        self.mask_test = (self.mask_test > (1 - num_positive_test / (self.n * self.n))).astype(int)
    ...
    def recompute_feature(self, G):
        ...
        random_subsets = get_random_subsets(G, c=2)
    ...
    def get_fullbatch_train(self):
        ...
        return (self.node_feature, self.adj_train, subset_dists, subset_features, self.mask_train)

    def get_fullbatch_test(self):
        subset_dists, subset_features = self.recompute_feature(self.G_train)
        return (self.node_feature, self.adj_train, subset_dists, subset_features, self.mask_test)